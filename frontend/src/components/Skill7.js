...
import jsonData from './questionData/SocialSkillsQuestions.json';
import './../App.css';
...
const ASSESS_SKILL = gql`
  mutation AssessUserSocialSkill($answers: SocialQuestionnaireInput!) {
    assessUserSocialSkill(answers: $answers) {
      success
      errors
    }
  }
...
function App() {
  ...
  const [assessUserSocialSkill] = useMutation(ASSESS_SKILL);
  ...
  const submitResponses = async () => {
    try {
      ...
      const response = await assessUserSocialSkill({
        variables: {
