...
RSpec.describe Commands::AssessUserArtAndCreativitySkill do
  let(:user) { create(:user) }
  let!(:skill) { create(:skill, name: 'Art and Creativity') }
  let(:answer) { 'answer for art and creativity skill assessment question' }
  let(:idea_count) { 6 }

  subject(:result) do
    described_class.call(user_id: user.id, idea_count:, answer:)
  end
  ...
  describe '.call' do
    context 'when user has an art and creativity skill' do
    ...
    context 'when user does not have an art and creativity skill' do
    ...
    context 'when the number of ideas is less than four' do
      let(:idea_count) { 2 }
      ...
      it 'does not call OpenAI API' do
        expect_any_instance_of(described_class).not_to receive(:call_openai_api)
      end
    ...
    context 'when user does not exist' do
      it 'returns an error' do
        result = described_class.call(user_id: -1, idea_count:, answer:)
    ...
    context 'when art and creativity does not exist' do
      before { skill.destroy }

      it 'returns an error' do
        expect(result[:success]).to be false
        expect(result[:errors]).to include('Art and Creativity skill not found')
      end
    end
    ...
    context 'when OpenAI API returns an invalid response' do
      before do
        ...
        .and_return('Based on the answer the user possesses excellent art and creativity skills') 
      end
