...
RSpec.describe Commands::AssessUserProjectManagementSkill do
  let(:user) { create(:user) }
  let!(:skill) { create(:skill, name: 'Project management') }
  let(:answer) { 'answer for project management skill assessment question' }
  ...
  describe '.call' do
    context 'when user has a project management skill' do
    ...
    context 'when user does not have a project management skill' do
    ...
    context 'when project management does not exist' do
      before { skill.destroy }

      it 'returns an error' do
        expect(result[:success]).to be false
        expect(result[:errors]).to include('Project management skill not found')
      end
    end
    ...
    context 'when OpenAI API returns an invalid response' do
      before do
        ...
        .and_return('Based on the answer the user possesses excellent project management skills') 
      end
